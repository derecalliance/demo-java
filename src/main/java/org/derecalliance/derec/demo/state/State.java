...
public class State {
    ...
    <T> void updateFromLibMethod(List<T> fromLib, ObservableList<T> stateVar) {
        ...
        for (T libElem : fromLib) {
            if (!stateVar.contains(libElem)) {
                ...
                //                        if
                // (libStatus.getId().getPublicEncryptionKey().equals(stateStatus.getId().getPublicEncryptionKey())) {
                //                            System.out.println("For user: " + libStatus.getId().getName() + " or " +
                // stateStatus.getId().getName() +
                //                                    "Status = " + libStatus.getStatus() + " or " +
                // stateStatus.getStatus());
                //                            if (libStatus.getStatus() != stateStatus.getStatus()) {
                //                                System.out.println("updateHelperOrSharerStatusFromLibMethod detected
                // status change " +
                //                                        "from " +libStatus.getStatus() + " to " +
                // stateStatus.getStatus());
                //                                stateVar.remove(stateElem);
                //                                stateVar.add(libElem);
                //                            }
                //                        } else {
                //                            System.out.println("For user: " + libStatus.getId().getName() + " != " +
                // stateStatus.getId().getName());
                //                        }
                //                    }
                //                }
    ...
    public void updateSharerFromLib() {
        ...
        if (userSelections.secret != null) {
            //            updateFromLibMethod((List<DeRecHelperStatus>) userSelections.secret.getHelperStatuses(),
            // helperStatusesForSelectedSecret);
            ...
            //            updateFromLibMethod((List<DeRecVersion>) new
            // ArrayList<>(userSelections.secret.getVersions().values()),
            ...
            for (var entry : userSelections.secret.getVersions().entrySet()) {
                ...
                // String(entry.getValue().getProtectedValue(), StandardCharsets.UTF_8) + "\n\n");
                //
                //                Text status = new Text(entry.getValue().isProtected() ? "This version is protected" :
                // "This version is not protected yet");
                ...
                //                updatedSharerTabVersionsContents.add(new DisplayEntry(title, textFlow.toString(),
                // entry.getValue()));
